...
class CLIPVisionTower(nn.Module):
    ...
    def get_frequency_subimg(self, image, cutoff_frequency = 30):
        ...
        def apply_filter(image_tensor, filter_func, cutoff, type='A'):
            ...
            seed = torch.rand(bz, 3, 336, 336).to(device=image_tensor.device) * 0.7
            
            seedA = torch.rand(bz, 3, 336, 336).to(device=image_tensor.device) * 0.8
            seedB = torch.rand(bz, 3, 336, 336).to(device=image_tensor.device) * 0.2

            # seed = torch.ones(bz, 3, 336, 336).to(device=image_tensor.device) 
            if type=='A':
                fshift_filtered = fshift * filter_mask * seedA
            elif type=='B':
                fshift_filtered = fshift * filter_mask * seedB
            elif type=='C':

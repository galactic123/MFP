...
class CLIPVisionTower(nn.Module):
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        self.choice_d = nn.Parameter(torch.rand(32, 576, 1024), requires_grad=True)
        self.choice_o = nn.Parameter(torch.rand(32, 576, 1024), requires_grad=True)
        if not delay_load:
            self.load_model()
        else:
    ...
    def forward(self, images, highpass_images, lowpass_images):
        ...
        if type(images) is list:
            ...
            image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, begin_layer=0, end_layer=6).hidden_states[-1]

            highpass_image_forward_outs = self.vision_tower(highpass_images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, begin_layer=0, end_layer=6).hidden_states[-1]
            lowpass_image_forward_outs = self.vision_tower(lowpass_images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, begin_layer=0, end_layer=6).hidden_states[-1]
            # print(highpass_image_forward_outs.size(), lowpass_image_forward_outs.size())  

...
class CLIPVisionTower(nn.Module):
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        self.choice_d = nn.Parameter(torch.ones(32, 576, 1024), requires_grad=True)
        self.choice_o = nn.Parameter(torch.ones(32, 576, 1024), requires_grad=True)
    ...
    def get_frequency_subimg(self, image, cutoff_frequency = 30):
        ...
        def apply_filter(image_tensor, filter_func, cutoff, type='A'):
            ...
            seed = torch.rand(bz, 3, 336, 336).to(device=image_tensor.device) * 2 - 1
            if type=='A':
    ...
    def forward(self, images, highpass_images, lowpass_images):
        ...
        if type(images) is list:
            ...
            image_features = self.deepfuse(image_features, auxiliary_features) * self.choice_d + image_features * self.choice_o

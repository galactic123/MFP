...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in self.list_data_dict[i]:
            ...
            data_dict['lowpass_images'] = torch.zeros(3, crop_size['height'], crop_size['width'])
        return data_dict
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'images' in instances[0]:
            ...
            highpass_images = [instance['highpass_images'] for instance in instances]
            lowpass_images = [instance['lowpass_images'] for instance in instances]
            if all(x is not None and x.shape == images[0].shape for x in images):
                batch['images'] = torch.stack(images)
                batch['highpass_images'] = torch.stack(highpass_images)
                batch['lowpass_images'] = torch.stack(lowpass_images)
...
def train():
    ...
    if model_args.vision_tower is not None:
        ...
        # if model_args.tune_qkv_matrix:
        for p in model.get_vision_tower().w_q.parameters():
            p.requires_grad = True
        for p in model.get_vision_tower().w_k.parameters():
            p.requires_grad = True
        for p in model.get_vision_tower().w_v.parameters():
            p.requires_grad = True
        for p in model.get_vision_tower().filter_A.parameters():
            p.requires_grad = True   
        for p in model.get_vision_tower().filter_B.parameters():
            p.requires_grad = True
        for p in model.get_vision_tower().filter_C.parameters():
            p.requires_grad = True
        for p in model.get_vision_tower().choice_d.parameters():
            p.requires_grad = True
        for p in model.get_vision_tower().choice_o.parameters():
            p.requires_grad = True

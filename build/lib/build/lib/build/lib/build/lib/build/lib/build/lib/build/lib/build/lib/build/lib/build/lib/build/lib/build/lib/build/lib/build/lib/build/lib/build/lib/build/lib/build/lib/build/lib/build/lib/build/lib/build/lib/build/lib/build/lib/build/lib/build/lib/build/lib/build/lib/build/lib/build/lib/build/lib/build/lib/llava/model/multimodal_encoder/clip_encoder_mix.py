...
class CLIPVisionTower(nn.Module):
    ...
    def get_frequency_subimg(self, image, cutoff_frequency = 30):
        ...
        def apply_filter(image_tensor, filter_func, cutoff, type='A'):
            ...
            seed = torch.rand(bz, 3, 336, 336).to(device=image_tensor.device) * 2 
            if type=='A':
                fshift_filtered = fshift * filter_A * seed
            elif type=='B':
                fshift_filtered = fshift * filter_B * seed
            elif type=='C':
                fshift_filtered = fshift * filter_C * seed
    ...
    def forward(self, images, highpass_images, lowpass_images):
        ...
        if type(images) is list:
            ...
            image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, begin_layer=0, end_layer=20).hidden_states[-1]

            highpass_image_forward_outs = self.vision_tower(highpass_images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, begin_layer=0, end_layer=20).hidden_states[-1]
            band_image_forward_outs = self.vision_tower(middle_images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, begin_layer=0, end_layer=20).hidden_states[-1]
            lowpass_image_forward_outs = self.vision_tower(lowpass_images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, begin_layer=0, end_layer=20).hidden_states[-1]
            ...
            auxiliary_features = torch.concat((highpass_image_forward_outs.unsqueeze(dim=-1), band_image_forward_outs.unsqueeze(dim=-1), lowpass_image_forward_outs.unsqueeze(dim=-1)), dim=3).transpose(-1, -2)
            ...
            image_features = self.vision_tower(image_features, output_hidden_states=True, begin_layer=20, end_layer=None, use_pre=False)

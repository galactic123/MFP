...
class CLIPVisionTower(nn.Module):
    ...
    def forward(self, images, highpass_images, lowpass_images):
        ...
        if type(images) is list:
            ...
            image_forward_outs = self.vision_tower(images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, begin_layer=0, end_layer=10).hidden_states[-1]

            highpass_image_forward_outs = self.vision_tower(highpass_images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, begin_layer=0, end_layer=10).hidden_states[-1]
            lowpass_image_forward_outs = self.vision_tower(lowpass_images.to(device=self.device, dtype=self.dtype), output_hidden_states=True, begin_layer=0, end_layer=10).hidden_states[-1]
            ...
            image_features = self.vision_tower(image_features, output_hidden_states=True, begin_layer=10, end_layer=None, use_pre=False)
